...
export interface LiquidEtherProps {
  ...
  /** URL of an image the fluid distorts (cover-fit). Omit for the classic palette-only look. */
  imageSrc?: string;
  /** How far the velocity field displaces the image UVs. ~0.1–0.5. */
  imageDistortion?: number;
  /** How strongly palette color tints the image where the fluid moves (0–1). */
  imageTint?: number;
}
...
`;
    ...
    uniform float imageDistortion;
    uniform float imageTint;
    ...
    vec4 base = bgColor;
    if (hasImage > 0.5) {
        vec2 iuv = (uv - 0.5) * coverScale + 0.5;
        iuv -= vel * imageDistortion;
        base = texture2D(imageTex, clamp(iuv, 0.0, 1.0));
    }
    float tint = lenv * mix(1.0, imageTint, hasImage);
    vec3 outRGB = mix(base.rgb, c, tint);
    float outA = mix(base.a, 1.0, tint);
    ...
    class Output {
        ...
        this.output = new THREE.Mesh(
          ...
          new THREE.RawShaderMaterial({
            ...
            uniforms: {
              ...
              imageTex: { value: null },
              hasImage: { value: 0 },
              imageDistortion: { value: imageDistortion },
              imageTint: { value: imageTint },
      ...
      loadImage(src: string) {
        new THREE.TextureLoader().load(src, tex => {
          ...
          const u = (this.output.material as THREE.RawShaderMaterial).uniforms;
          u.imageTex.value = tex;
          u.hasImage.value = 1;
          this.updateCoverScale();
        });
      }
      // Cover-fit: scale sim UVs so the image fills the container without
      // stretching, cropping the overflowing axis (CSS object-fit: cover).
      updateCoverScale() {
        const u = (this.output.material as THREE.RawShaderMaterial).uniforms;
        const containerAspect = Common.aspect || 1;
        if (containerAspect > this.imageAspect) {
          u.coverScale.value.set(1, this.imageAspect / containerAspect);
        } else {
          u.coverScale.value.set(containerAspect / this.imageAspect, 1);
        }
      }
      resize() {
        this.simulation.resize();
        if (this.imageTexture) this.updateCoverScale();
      }
      render() {
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      dispose() {
        try {
          ...
          Mouse.dispose();
          if (this.output?.imageTexture) this.output.imageTexture.dispose();
          if (Common.renderer) {
